...
module.exports = {
  map: abilityMap,
  setupAbilities() {
    console.groupCollapsed("-- Yetenekler --");
    console.log("Yetenekler ayarlanıyor...");

    for(const ability of Object.values(abilityData)) {
      abilityMap.set(ability.id, ability.func);
      console.log(`> Yetenek ${ability.name} ayarlandı !`);
    }
    ...
    console.log("Yetenekler ayarlandı !");
    console.groupEnd();
  },
...
function phys_low_health_damage(exeData, quantity) {
  ...
  for(const target of targets) {
      let damage = Math.floor((quantity * (caster.stats.strength / Math.max(1,target.stats.resistance)) + 2) / 2);
