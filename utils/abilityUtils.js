...
exports.selectActiveAbility = async function(user_id, query) {
    ...
    if(inventory.activeAbilities.length >= 99) {
        console.error("[DEBUG] 99 Active Abilities already used by user " + user_id + ". (ACTIVE_ABILITY_MAX_SIZE)");
        return "max_size";
    }
...
exports.sendModal = function(interaction, select, userId) {
    if(interaction.user.id != userId) {
        interaction.reply({content: "Bu senin envanterin değil! Kullanmak için, `mc` yazabilirsin.", ephemeral: true});
        return;
    }
    ...
    if(select == true) {
        ...
        abilityInput.setLabel("Kullanmak istediğin yeteneğin numarasını gir.")
    } else {
        modal.setCustomId("unselectAbility")
        abilityInput.setLabel("Çıkarmak istediğin yeteneğin numarasını gir.")
    }
...
exports.receiveModal = async function(interaction, select) {
    ...
    if(typeof ret != "string") {
        ...
        switch(ret) {
            case "not_learned":
                interaction.reply({content: "Bu yeteneği henüz öğrenmedin!", ephemeral: true});
                break;
            case "max_size":
                interaction.reply({content: "You have already selected 99 abilities!", ephemeral: true});
                break;
            case "already_active":
                interaction.reply({content: "Bu yeteneği zaten kullanıyorsun!", ephemeral: true});
                break;
            case "not_selected":
                interaction.reply({content: "Bu yeteneği zaten kullanmıyorsun!", ephemeral: true});
                break;
            default:
                interaction.reply({content: "Bir sorun oluştu. Geliştiriciye bildiriniz.", ephemeral: true});
        }
    }
}
